# 🤖 Telegram Tarif Bot

Телеграм-бот для записи на онлайн-консультации с оплатой через ЮKassa, опросниками и админ-панелью.

## 📋 Тарифы

| Тариф | Цена | Особенности |
|-------|------|-------------|
| 💚 Базовый | 8 000 ₽ | Полный опросник + аюрвед + фото анализов + выбор слота |
| 💎 Сопровождение | 20 000 ₽ | Расширенная консультация с полным сопровождением |
| 🔄 Повторная | 5 000 ₽ | Без опросников, фото опционально |
| 🌿 Лайт | 3 000 ₽ | Только аюрвед-опросник (по полу) |

---

## 🚀 Установка и запуск

### Требования

- **Сервер**: Ubuntu 24.04 (или любой Linux с Docker)
- **Docker**: версия 20.10+
- **Docker Compose**: версия 2.0+
- **Telegram Bot Token**: получить у [@BotFather](https://t.me/BotFather)
- **ЮKassa**: аккаунт продавца на [yookassa.ru](https://yookassa.ru)

### Шаг 1. Клонирование репозитория

```bash
git clone https://github.com/kiore73/telegram-tarif-bot.git
cd telegram-tarif-bot
```

### Шаг 2. Создание Telegram-бота

1. Откройте [@BotFather](https://t.me/BotFather) в Telegram
2. Отправьте `/newbot`
3. Введите имя и username бота
4. Скопируйте полученный **токен** (формат: `123456:ABC-DEF1234...`)

### Шаг 3. Настройка ЮKassa

1. Зарегистрируйтесь на [yookassa.ru](https://yookassa.ru)
2. Создайте магазин в личном кабинете
3. Перейдите в **Настройки → Ключи API**
4. Скопируйте **Shop ID** и **Секретный ключ**
5. Для тестирования используйте [тестовый магазин](https://yookassa.ru/developers/payment-acceptance/testing-and-going-live/testing)

### Шаг 4. Настройка переменных окружения

```bash
cp .env.example .env
nano .env
```

Заполните файл `.env`:

```env
# === ОБЯЗАТЕЛЬНЫЕ ===

# Токен бота от @BotFather
BOT_TOKEN=123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11

# Telegram ID администраторов (через запятую)
# Узнать свой ID: отправьте /start боту @userinfobot
ADMIN_IDS=123456789,987654321

# PostgreSQL (менять только если меняете пароль в docker-compose)
DATABASE_URL=postgresql+asyncpg://botuser:botpassword@postgres:5432/tarif_bot

# === ЮKASSA ===
YOOKASSA_SHOP_ID=your_shop_id
YOOKASSA_SECRET_KEY=your_secret_key
YOOKASSA_RETURN_URL=https://t.me/your_bot_username
YOOKASSA_DEFAULT_RECEIPT_EMAIL=receipt@yourdomain.com
YOOKASSA_VAT_CODE=1
YOOKASSA_PAYMENT_MODE=full_payment
YOOKASSA_PAYMENT_SUBJECT=service
YOOKASSA_ENABLED=true

# === ТАРИФЫ (цены в рублях) ===
TARIFF_BASIC_PRICE=8000
TARIFF_EXTENDED_PRICE=20000
TARIFF_REPEAT_PRICE=5000
TARIFF_LITE_PRICE=3000

# === ДОПОЛНИТЕЛЬНО ===
TIMEZONE=Europe/Moscow
UPLOAD_DIR=./uploads
PHOTO_RETENTION_DAYS=30

# === ВЕБХУК ЮKassa (автоматическое подтверждение оплаты) ===
WEBHOOK_PORT=8080
YOOKASSA_WEBHOOK_SECRET=your_webhook_secret
```

> ⚠️ **Важно**: `YOOKASSA_RETURN_URL` должен быть ссылкой на вашего бота (`https://t.me/имя_бота`), чтобы после оплаты пользователь вернулся в чат.

### Шаг 5. Настройка вебхуков ЮKassa (SSL + домен)

Для автоматического подтверждения оплаты ЮKassa отправляет POST-запросы на ваш сервер. Для этого **нужен домен с SSL-сертификатом**.

#### 5.1. Получение домена

Привяжите домен (например, `bot.yourdomain.ru`) к IP-адресу вашего сервера через A-запись в DNS.

#### 5.2. Установка Nginx + Let's Encrypt

```bash
# Установка Nginx и Certbot
sudo apt update
sudo apt install -y nginx certbot python3-certbot-nginx

# Создание конфига Nginx
sudo nano /etc/nginx/sites-available/tarif-bot
```

Содержимое файла:
```nginx
server {
    listen 80;
    server_name bot.yourdomain.ru;

    location /webhook/ {
        proxy_pass http://127.0.0.1:8080;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }
}
```

```bash
# Активация конфига
sudo ln -s /etc/nginx/sites-available/tarif-bot /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx

# Получение SSL-сертификата (бесплатно)
sudo certbot --nginx -d bot.yourdomain.ru
```

#### 5.3. Настройка URL в ЮKassa

1. Откройте [Личный кабинет ЮKassa](https://yookassa.ru/my) → **Настройки** → **HTTP-уведомления**
2. Укажите URL: `https://bot.yourdomain.ru/webhook/yookassa`
3. Включите события: **payment.succeeded**, **payment.canceled**
4. Сохраните


### Шаг 6. Запуск через Docker Compose

```bash
# Собрать и запустить контейнеры (бот + PostgreSQL)
docker-compose up -d --build

# Проверить что контейнеры работают
docker-compose ps
```

Вы должны увидеть два контейнера в статусе `Up`:
```
NAME                    STATUS
telegram-tarif-bot-bot-1       Up
telegram-tarif-bot-postgres-1  Up
```

### Шаг 7. Инициализация базы данных

```bash
# Создать миграцию (первый раз)
docker-compose exec bot alembic revision --autogenerate -m "initial"

# Применить миграцию
docker-compose exec bot alembic upgrade head
```

### Шаг 8. Проверка работы

1. Откройте вашего бота в Telegram
2. Отправьте `/start` — должно появиться приветствие с выбором тарифов
3. Отправьте `/admin` — должна появиться панель администратора (только для ADMIN_IDS)

---

## 🔧 Управление

### Логи бота
```bash
# Все логи
docker-compose logs bot

# Только последние 50 строк в реальном времени
docker-compose logs -f --tail 50 bot
```

### Перезапуск
```bash
docker-compose restart bot
```

### Остановка
```bash
docker-compose down
```

### Обновление кода
```bash
git pull
docker-compose up -d --build
```

### Очистка старых фото
```bash
# Вручную (удалит фото старше PHOTO_RETENTION_DAYS дней)
docker-compose exec bot python scripts/cleanup_photos.py

# Автоматически через cron (каждый день в 3:00)
# Добавьте в crontab -e:
# 0 3 * * * cd /path/to/telegram-tarif-bot && docker-compose exec -T bot python scripts/cleanup_photos.py
```

---

## 🛠 Админ-панель

Команда `/admin` доступна только пользователям, чьи Telegram ID указаны в `ADMIN_IDS`.

| Функция | Описание |
|---------|----------|
| ➕ Создать слот | Ввод даты → времени → длительности (30–60 мин) |
| 🗑 Удалить слот | Выбор из списка будущих слотов |
| 📋 Свободные слоты | Просмотр всех доступных слотов |
| 📊 История записей | Последние 20 записей |

---

## 📁 Структура проекта

```
telegram-tarif-bot/
├── bot/
│   ├── main.py              # Точка входа, Dispatcher
│   ├── keyboards.py         # Все inline-клавиатуры
│   ├── texts.py             # Текстовые константы (RU)
│   ├── middlewares.py        # DB session middleware
│   ├── handlers/
│   │   ├── start.py         # /start + выбор тарифа
│   │   ├── payment.py       # Проверка оплаты ЮKassa
│   │   ├── intake.py        # Ввод данных (имя, возраст, вес)
│   │   ├── questionnaire.py # Движок опросников
│   │   ├── photos.py        # Загрузка фото анализов
│   │   ├── slots.py         # Выбор слота + бронирование
│   │   ├── gender.py        # Выбор пола (для лайт-тарифа)
│   │   └── admin.py         # Админ-панель
│   ├── services/
│   │   ├── questionnaire_engine.py  # Парсер .md опросников
│   │   ├── yookassa_service.py      # Интеграция с ЮKassa
│   │   ├── slot_service.py          # CRUD слотов
│   │   ├── booking_service.py       # Создание записей
│   │   ├── photo_service.py         # Сохранение фото
│   │   └── notification_service.py  # Уведомления админам
│   └── states/
│       └── user_states.py   # FSM-состояния
├── db/
│   ├── base.py              # SQLAlchemy engine + Base
│   ├── models.py            # Модели: User, Payment, Booking, Slot...
│   └── migrations/          # Alembic миграции
├── config/
│   └── settings.py          # Pydantic Settings из .env
├── scripts/
│   └── cleanup_photos.py    # Крон-скрипт очистки фото
├── Dockerfile
├── docker-compose.yml
├── requirements.txt
├── .env.example
└── README.md
```

---

## 🔄 Поток работы бота

### Базовый / Сопровождение
```
/start → Выбор тарифа → Оплата ЮKassa → Имя → Фамилия → Возраст → Вес
→ Базовый опросник (73 вопроса) → Аюрвед-опросник (по полу)
→ Загрузка фото → Выбор слота → Подтверждение → Уведомление админу
```

### Повторная
```
/start → Выбор тарифа → Оплата → Имя → Фамилия → Возраст
→ Фото (опционально) → Выбор слота → Подтверждение
```

### Лайт
```
/start → Выбор тарифа → Оплата → Имя → Фамилия → Возраст → Вес
→ Выбор пола → Аюрвед-опросник → Выбор слота → Подтверждение
```

---

## 🧰 Технологии

| Компонент | Технология |
|-----------|-----------|
| Язык | Python 3.11+ |
| Telegram API | aiogram 3.x |
| ORM | SQLAlchemy 2.0 (async) |
| База данных | PostgreSQL 16 + asyncpg |
| Миграции | Alembic |
| Оплата | ЮKassa SDK |
| Конфигурация | Pydantic Settings |
| Деплой | Docker + Docker Compose |

---

## ❓ Решение проблем

| Проблема | Решение |
|----------|---------|
| Бот не отвечает | Проверьте `docker-compose logs bot` на ошибки |
| `BOT_TOKEN` ошибка | Убедитесь, что токен корректный и бот не заблокирован |
| БД не подключается | Проверьте что postgres контейнер запущен: `docker-compose ps` |
| Оплата не работает | Проверьте `YOOKASSA_SHOP_ID` и `YOOKASSA_SECRET_KEY`, используйте тестовый магазин |
| Миграция падает | Убедитесь что БД доступна: `docker-compose exec postgres psql -U botuser -d tarif_bot` |
